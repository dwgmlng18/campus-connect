...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    if (error) {
      return NextResponse.json({ success: false, message: `Gagal menyetujui event: ${error.message}` }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: "Event berhasil disetujui dan kini tayang di halaman publik." });
  } catch (error: any) {
