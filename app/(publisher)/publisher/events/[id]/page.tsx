...
export default async function PublisherEventDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <div className={`mb-8 p-5 rounded-2xl border text-sm font-semibold flex items-start gap-4 ${
        approvalInfo.status === "approve"
          ? "bg-accent-50 text-accent-700 border-accent-200 dark:bg-accent-950/20 dark:border-accent-900/50"
        ...
        <div>
          <p className="font-bold text-base">
            Status Event: {approvalInfo.status === "approve" ? "Disetujui & Tayang Publik" : approvalInfo.status === "reject" ? "Ditolak Admin" : "Menunggu Review Admin"}
          </p>
          {approvalInfo.status === "reject" && approvalInfo.note && (
            <p className="text-xs font-medium mt-1 text-red-600/90 dark:text-red-400/90">
              Catatan Penolakan: "{approvalInfo.note}"
            </p>
          )}
          {approvalInfo.status === "pending" && (
            <p className="text-xs font-medium mt-0.5 text-amber-600/90 dark:text-amber-400/90">
              Event Anda sedang dalam antrean review oleh Superadmin. Pengunjung umum belum bisa melihat event ini.
            </p>
          )}
        </div>
      </div>
