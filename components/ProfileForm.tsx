...
export default function ProfileForm({ profile }: ProfileFormProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setProfileError("Ukuran logo maksimal adalah 5MB.");
        e.target.value = "";
        setLogoName("");
        setLogoPreview(null);
        return;
  ...
  const handleCancelLogo = () => {
    setLogoName("");
    setLogoPreview(null);
    const fileInput = document.getElementById("profile_org_logo") as HTMLInputElement;
    ...
    startTransition(async () => {
      try {
        ...
        if (res.success) {
          ...
          setLogoName("");
          setLogoPreview(null);
          // Reload page to refresh sidebar/header logo
          window.location.reload();
        } else {
          setProfileError(res.message || "Gagal memperbarui profil.");
        }
      } catch (err: any) {
        setProfileError("Terjadi kesalahan sistem saat memperbarui profil.");
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (res.success) {
          ...
          setPasswordError(res.message || "Gagal memperbarui password.");
        }
      } catch (err: any) {
        setPasswordError("Terjadi kesalahan sistem saat memperbarui password.");
      }
    });
  };

  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-md overflow-hidden transition max-w-2xl mx-auto w-full">
      {/* Tab Header */}
              ...
              <div>
                ...
                <div className="flex items-center justify-center w-full">
                  ...
                  {logoPreview ? (
                    <div className="flex items-center justify-between p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-900/30 w-full">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-accent-500">Logo baru siap diunggah</p>
                        </div>
                      </div>
                      <button
                        type="button"
                    ...
                    <label htmlFor="profile_org_logo" className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-primary-400 dark:hover:border-primary-600 rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-900/50 transition">
                      <div className="flex flex-col items-center justify-center pt-4 pb-4">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 px-4 text-center">
                          <span>Pilih berkas gambar logo baru (Maks 5MB)</span>
                        </p>
                      </div>
                    </label>
                  )}
